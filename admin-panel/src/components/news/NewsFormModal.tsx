...
export default function NewsFormModal({ news, isOpen, onClose, onSuccess }: NewsFormModalProps) {
  ...
  const [newsType, setNewsType] = useState<'content' | 'external'>('content');
  ...
  useEffect(() => {
    if (isOpen) {
      if (news) {
        ...
        setNewsType(news.externalUrl ? 'external' : 'content');
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (newsType === 'content' && !formData.content.trim()) {
      setError('İçerik zorunludur');
      return;
    }

    if (newsType === 'external' && !formData.externalUrl.trim()) {
      setError('Dış link zorunludur');
      return;
    }
    ...
    try {
      ...
      if (newsType === 'content') {
        ...
        body.externalUrl = formData.externalUrl.trim();
      ...
      if (isEditMode) {
        ...
        await newsService.createNews(body);
      }
        ...
        <div className="relative bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Haber Tipi <span className="text-red-500">*</span>
                </label>
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setNewsType(e.target.value as 'content' | 'external')}
                      className="w-4 h-4 text-slate-600"
                    />
                    <span className="text-sm text-gray-700">İçerik</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      value="external"
                      checked={newsType === 'external'}
                      onChange={(e) => setNewsType(e.target.value as 'content' | 'external')}
                      className="w-4 h-4 text-slate-600"
                    />
                    <span className="text-sm text-gray-700">Dış Link</span>
                  </label>
                </div>
              </div>
              ...
              {newsType === 'content' ? (
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Dış Link <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="url"
